	...
	<div class="row page-titles">
	    ...
	    <div class="col-md-7 align-self-center">
	        <ol class="breadcrumb">
	            ...
	            <li class="breadcrumb-item active">Top Up Group</li>
	        </ol>
	    </div>
	</div>
	        ...
	        <div class="col-12">
	            <div class="card">
	                <div class="card-body">
	                    <div class="row">
	                       	<div class="col-lg-12">
								<form method="GET" action="{{ URL('dashboard/top_up_group/search') }}" class="form-horizontal m-t-40">
									{{ csrf_field() }}
						...
						<br/>
						<div align="center">{{ Shwetech::add($link_top_up_group,'dashboard/top_up_group/add') }}</div>
						<br/>
	                    <table class="tablesaw table-striped table-hover table-bordered table" data-tablesaw-mode="columntoggle">
	                        <thead>
	                            <tr>
	                                ...
	                                <th scope="col" data-tablesaw-sortable-col data-tablesaw-sortable-default-col data-tablesaw-priority="4">Group</th>
	                                <th scope="col" data-tablesaw-sortable-col data-tablesaw-sortable-default-col data-tablesaw-priority="5">Agent</th>
	                        ...
	                        <tbody>
	                        	@php($no = 1)
	                        	@foreach($view_top_up_groups as $top_up_groups)
		                            <tr>
		                                <td>{{ $no }}</td>
		                                <td>{{ Shwetech::changeDBToDate($top_up_groups->date_top_ups) }}</td>
		                                <td>{{ $top_up_groups->time_top_ups }}</td>
		                                <td>{{ $top_up_groups->name_groups }}</td>
		                                <td>{{ $top_up_groups->name }}</td>
		                                <td>{{ $top_up_groups->credit_top_ups }}</td>
		                                <td width="5%">
		                                	<div class="input-group">
												<div class="input-group-btn">
													...
													<div class="dropdown-menu dropdown-menu-right action">
														{{ Shwetech::read($link_top_up_group,'dashboard/top_up_group/read/'.$top_up_groups->id_top_ups) }}
														{{ Shwetech::edit($link_top_up_group,'dashboard/top_up_group/edit/'.$top_up_groups->id_top_ups) }}
														{{ Shwetech::delete($link_top_up_group,'dashboard/top_up_group/delete/'.$top_up_groups->id_top_ups,$top_up_groups->name) }}
													</div>
												</div>
											</div>
		                                </td>
