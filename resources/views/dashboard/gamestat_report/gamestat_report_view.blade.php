	        ...
	        <div class="col-12">
	            <div class="card">
	                <div class="card-body">
	                    <div class="row">
	                       	<div class="col-lg-12">
								<form method="GET" action="{{ URL('dashboard/gamestat_report/search') }}" class="form-horizontal m-t-40">
									{{ csrf_field() }}
	                    ...
	                    <table class="tablesaw table-striped table-hover table-bordered table" data-tablesaw-mode="columntoggle">
	                        ...
	                        <tbody>
	                        	@php($no_group = 1)
	                        	@foreach($view_gamestat_reports as $gamestat_reports)
		                            <tr>
		                                <td>{{ $no_group.'. '.$gamestat_reports->name_groups }}</td>
		                                <td>{{ $gamestat_reports->credit_groups }}</td>
		                                @if(Auth::user()->level_systems_id != 3)
		                                	@if(Auth::user()->level_systems_id == 1)
		                                		<td colspan="6">
		                                			@php($id_master_agent 	= $gamestat_reports->sub_users_id)
		                                			@php($get_master_agent = \App\Master_user::where('id',$id_master_agent)->first())
		                                			{{ $get_master_agent->name.' - '.$gamestat_reports->name }}
		                                		</td>
		                                	@elseif(Auth::user()->level_systems_id == 2)
		                                		<td colspan="6">
		                                			{{ $gamestat_reports->name }}
		                                		</td>
		                                	@endif
		                                @else
			                                <td colspan="6"></td>
			                        ...
			                        @foreach($get_sessions as $sessions)
			                            ...
			                            @foreach($get_game as $game)
			                            	<tr>
			                            		...
			                            		<td>
													{{ Shwetech::detail($link_gamestat_report,'dashboard/gamestat_report/detail/'.$game->id_games) }}
			                            		</td>
			                            	</tr>
			                            	@php($no_game++)
			                            @endforeach
